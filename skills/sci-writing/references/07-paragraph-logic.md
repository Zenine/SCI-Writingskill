...
1. 每段有且只有一个功能(给背景 / 比较方法 / 报告关键结果 / 解释某一发现……),段内每句都推动该功能;与功能无关的句子删除或另起段。
...
| Old-to-New | 引言、文献综述 | 早期已知 → 近期前沿 → 本文问题;时间状语打头,引文按年份递进 | 抽出段内年份呈单调递增;每句主语是上句已出现的信息或其延伸 |
...
混用规则:引言与方法、结果的各小节默认 G-S;讨论小节可用 S-G 或 G-S-G;摘要 = 前 2–3 句 G-S,中间 2–3 句 P-S,末 2–3 句 S-G。
...
| this / these + 概括名词 | this process / this decrease / these findings / this discrepancy | 裸 This / It / They 开头视为错误;概括名词与首次出现时的名词一致(说了 a device 就写 this device) |
| 信号词 | However / Therefore / In addition 等直接告诉读者本句功能 | 先定关系类型再选词;用错比不用更糟;不必每句都用 |
| 分号 | 两句关系极紧且其中一句很短时代替句号 | 合并后重新检查总句长 |

### 连接词密度上限

1. 同一段落中以连接副词起句的句子不超过一半;超过即公式化,改用重叠重复或 this + 名词承接。
2. 不允许连续两句以同一类连接词起句(However … However …;Moreover … Furthermore …)。
3. 每个连接词都必须能归入"补充 / 让步 / 对比 / 因果 / 解释 / 举例 / 强调 / 顺序 / 总结"之一;归不进去的删除。
...
| 易混对 | 区别 | 规则 |
|---|---|---|
| because / as / since | because 只表原因;as 也可表 when,since 也可表 from that time | 三者都允许;有歧义时用 because(句中)或 Given that(句首);不把 because 一律判错 |
...
| however / but | 都可表对比或意外;but 偏口语 | 句首 But 改 However / In contrast / Unexpectedly;要强调"意外"时选 Nevertheless / Unexpectedly |
| moreover / in addition / besides | moreover 要求功能相同;besides 语气更强、偏论辩 | 默认 In addition / Furthermore;besides 不用 |
| on the other hand | 只用于同一对象的两个相反特征 | 比较两个不同对象改 in contrast |
| in fact / indeed | 只用于强化前句 | 可删则删;不作句间连接 |
| thus + -ing / which(表结果) | which 指代整句时所指不明 | 表结果用 ", thus / thereby + -ing";每句最多一个 -ing 从句 |
...
- Unlike X, …
- Conversely, …(仅"恰好相反")
- 例:Whereas the CNN missed most sub-centimetre nodules, the transformer detected 84% of them.
...
| 句首 And / But / So / Though | In addition / However / Therefore / …, even though |
| 用 and 硬连两个无逻辑关系的事实 | 拆句;按作者态度选 Notably / Interestingly / In addition |
| meanwhile 表对比 | whereas / while;真同时用 simultaneously |
| on the contrary 表一般对比 | in contrast / in comparison |
| Moreover 引出功能不同的句子 | 换 In addition,或先确认本句与上句同为理由/例子 |
| Therefore 之前没有明示原因 | 先补原因句,再用 Therefore |
| 裸 This / It / They 起句 | this + 概括名词;或重复具体名词 |
| which 指代整句或前有两个候选名词 | 改 thus / thereby + -ing;或移动 which 紧跟唯一先行词 |
| 一句里塞两个以上 -ing 从句 | 一句一个 -ing;其余拆句 |
| 为避免重复换新名词指同一对象 | 重复同一名词;学术写作鼓励重复 |
| 转折词放句末 | 移到句首 |
| in fact / actually / as we all know 作连接 | 删除 |
...
9. 每个连接词都能归入九类逻辑关系之一。✓/✗
10. 同一段以连接副词起句的句子不超过一半;无连续两句同类连接词。✓/✗
11. Moreover / Therefore / For example / In other words 各自满足约束语义。✓/✗
12. 句首无 And / But / So / Though;全文无 meanwhile。✓/✗
13. on the contrary、on the other hand 只用于同一对象或同一议题。✓/✗
14. 所有 This / These 后紧跟概括名词;It / They 所指唯一。✓/✗
15. 表结果的关系用 thus / thereby + -ing,每句最多一个 -ing 从句。✓/✗
