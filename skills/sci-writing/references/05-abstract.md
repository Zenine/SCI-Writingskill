...
各步句数是上限而非配额:总句数以 8–12 句、200–250 词为界(Nature 式含步 9 时可到 12 句),超出时按细则 13 合并。
...
5. 结果句(步 6)单独不超过摘要一半;方法与结果句(步 5–6)合计不超过 60%;通篇只是结果清单的摘要不合格。原稿背景与含义材料极少、压不到 60% 时,不编背景凑字:先删方法细节(划分比例、变量数可省),仍超则保留超出并在自检写明原因与百分数,进「建议补充」请作者补背景或含义。
...
2. 已核对目标期刊对摘要的字数、形态(一般 / 结构化 / Nature 式)与小标题要求(期刊未知时标 N/A)。✓/✗
...
14. 同一对象全篇一个名称。✓/✗
15. 摘要特例:平均句长 ≤ 25 词,单句 ≤ 40 词(其余数字见 `09a-metrics-table.md`)。✓/✗
