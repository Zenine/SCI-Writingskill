...
- 标题、关键词、摘要、正文结论使用同一套核心术语(见 `05-abstract.md`);关键词与标题的分工见关键词规则 2。
...
样本不足或无联网时的兜底:拿不到 20 条时降为 ≥ 8 条,并在「理解确认」注明样本量不足;一条都没有时按本文件默认值(约 12 词、无缩写、名词短语、不用冒号)起草,画像各项标"默认值,请确认",检索类动作(规则 9–10)进「建议补充」请作者执行。

| 步 | 做什么 | 产出 / 检查项 |
|---|---|---|
| 1 量长度 | 数样本标题词数,记平均值与波动范围 | 自己的标题偏离均值超过 50% 时要能说明理由;书A 给的经验值约 12 词,以自己统计的样本均值为准 |
...
6. 名词短语是默认形式,合法且不扣分;在此基础上优先让标题含动词或动作名词(X reduces Y / Reduction of Y by X),只命名对象的名词堆叠最弱。完整句子式标题(有主语、动词,一般现在时)只在目标期刊有先例且结论强而明确时使用;结论未坐实时用带情态动词的句子或退回名词短语。
7. 名词短语的中心名词在最右,左侧名词都在当形容词用,链越长关系越说不清。
8. 介词 for / with / in / of 语义弹性大;with 尤其易歧义,优先换 using / based on / combined with。
9. 标题中的关键术语必须是当前通用说法:用 Google Scholar 带引号与不带引号各搜一次,确认未被新命名取代;无联网时进「建议补充」请作者执行。
...
2. 核心术语(研究对象、方法名)可与标题重合,但关键词表中至少一半是标题里没有的词,走两种目的之一:扩大读者面(比标题更宽的上位词)或补充技术术语(吸引本领域专家)。
...
3. 结果节:优先用动词说出发现的关系或现象(X causes Y / X suppresses Y / X does not affect Y),形式按目标期刊(与 `03-results.md` 规则 16 一致);期刊只允许短语式小标题时,用含关系的名词短语(Effects of X on Y / Association between X and Y / Suppression of Y by X),两种形式下都不得只给主题名词。
...
- 例:2.3 Assessment of retinal image quality

**结果节二级标题**(优先含动词;短语式期刊用含关系的名词短语)
- X causes / induces Y
...
- Effects of X on Y / Association between X and Y(短语式)
...
| 句子式标题但期刊无先例 | 改为含动作名词的名词短语标题 |
| 关键词全部照抄标题 | 至少一半换成上位词或补充技术术语 |
