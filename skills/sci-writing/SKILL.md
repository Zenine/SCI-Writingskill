...
## 总流程(细节见 `references/00-workflow.md`)

1. 故事线:四问(数据 → 信息 → 新知识 → 新理解)+ 一句话张力陈述(前人认为 X,本文表明 Y)。
2. 骨架:引言/结果/讨论三组二级标题一一对应,方法标题含动作,结果标题含动词。
3. 分节填充:按 01–06 号文件的功能步写各节,先给结构再填内容。
4. 段落逻辑:一段一模型,句间显式承接(07)。
5. 句子与动词:时态按事实地位,语态按所有权可辨认,清理藏动作写法(08)。
6. 风格与逐句扫描:简洁与学术口径(09),中国作者错误清单(10)。
7. 自检与投稿:总清单、声明、选刊、审稿回复(11、12)。
...
| 写或改 Methods / Materials and Methods | `references/02-methods.md` | 08、11(伦理声明) |
...
| 写或压缩 Abstract / Highlights / 结构化摘要 | `references/05-abstract.md` | 06 |
...
| "wordy" / "informal" / 句子太长 / etc. / 问句 / 缩写 | `references/09-academic-style.md` | 10 |
...
一次请求涉及多节时,按总流程顺序逐节处理,每节各读其文件;不要凭记忆写规则。

## 硬规则(任何场景都适用)

1. **结构先于句子**:没有故事线与二级标题就不写句子;用户只要润色时,先用一句话复述你理解的故事线再改。
2. **每节先给结构再填内容**:输出某一节前,先列该节的功能步(如引言四步),再逐步填;用户能看到每段对应哪一步。
3. **不编造**:不添加原稿没有的数据、方法细节、选择理由、引用、机构名;发现缺关键信息(缺失值处理、随机种子、批号、选某方法的理由等),在输出末尾以「建议补充」列出,不写进正文。本条优先于任何 reference 中"应当补充说明"的细则:细则要求而原稿没给的内容,一律进「建议补充」。
4. **不编引用**:不生成任何文献条目;需要引用处用 `[REF]` 占位并说明该引什么。需要占位的是:命名算法与模型(XGBoost、U-Net)、专用软件包与工具、非标准或改自他人的方法、引言与讨论中的具体前人结论;教科书级通用方法(逻辑回归、t 检验、五折交叉验证)不占位。
5. **所有权可辨认**:每个发现句能看出是本研究得出(过去时 / we / this study / here 至少一项);无施动被动句必须有定位词。
6. **强度与证据匹配**:每个结论句只留一个避险词;相关性数据不用 cause;摘要与结论的强度不得高于结果节。
7. **术语一致**:同一对象全篇只用一个名称;缩写首次出现给全称;不做同义词替换。
...
1. **理解确认**(1–3 句):故事线、目标期刊/读者、本次处理的节。
2. **结构**:该节功能步清单,或改写时的段落功能标注。
3. **正文**:英文稿。改写时给改后全文,不给逐句对照,除非用户要求。
4. **改动说明**(按需):关键改动及其依据的规则,一条一句。
5. **自检结果**:固定报五项核心(时态、语态/所有权、结构、强度匹配、术语缩写),涉及 Methods 时加伦理声明项;每项 ✓/✗ 加一句。此外只列出本节 reference 自检清单中判为 ✗ 的条目(附编号),✓ 的不逐条罗列。
