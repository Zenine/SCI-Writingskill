...
| 写或压缩 Abstract / Highlights / 结构化摘要 | `references/05-abstract.md` | 06(仅术语一致条)、09(口径表) |
...
一次请求涉及多节时,按总流程顺序逐节处理,每节各读其文件;不要凭记忆写规则。路由表「再读」列是最低要求:只写某一节时,正文起草完仍要按 10 号清单做一遍逐句扫描(09 按需),扫描结果并入自检;凡自检涉及句长、段长数字,读 `09-academic-style.md` 的口径表。信件类文本(投稿信、回复信)同样视为"一节":结构 = `12-submission-checklist.md`「回复信的结构」给的骨架,逐句扫描照做,自检 = 核心五项 + 12 的 G 组(审稿回复)或 F 组(投稿信)中 ✗ 的条目,衔接提示写"信与稿件修改位置的对应"。
